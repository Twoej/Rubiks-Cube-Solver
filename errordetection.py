...
error_move_casting = {
    ...
    str([squarei('D', 2), squarei('F', 8), squarei('R', 6)]): [squarei('F', 8), squarei('R', 6), squarei('D', 2)],
    ...
    str([squarei('D', 3), squarei('L', 7)]): [squarei('L', 7), squarei('D', 3)],
    str([squarei('D', 5), squarei('R', 7)]): [squarei('D', 5), squarei('R', 7)],
    str([squarei('F', 3), squarei('L', 5)]): [squarei('F', 3), squarei('L', 5)],
    str([squarei('F', 5), squarei('R', 3)]): [squarei('F', 5), squarei('R', 3)],
    str([squarei('B', 5), squarei('L', 3)]): [squarei('L', 3), squarei('B', 5)],
    str([squarei('B', 3), squarei('R', 5)]): [squarei('B', 3), squarei('R', 5)]
    }
   
error_fix_stickers = [[squarei('F', 8), squarei('R', 6), squarei('D', 2)], [squarei('F', 5), squarei('R', 3)]]


reverse_turn_direction = {"U": "U'", "D": "D'", "L": "L'", "R": "R'", "F": "F'", "B": "B'", "U'": "U", "D'": "D", "L'": "L", "R'": "R", "F'": "F", "B'": "B"}

def reverse_moves(moves):
	list_moves = moves.split()
	new_moves = ""
	for i in range(len(list_moves)):
		if list_moves[i] in reverse_turn_direction.keys():
			list_moves[i] = reverse_turn_direction[list_moves[i]]
	for i in range(len(list_moves) - 1, -1, -1):
		new_moves += str(list_moves[i])
		new_moves += " "
	return new_moves.rstrip()
